...
function sendEmail(event) {
  ...
  const hasErrors = fullName.classList.contains("error") ||
  ...
  if (!hasErrors) {
    // NOTE: For production, move Email.send() to a backend service to avoid exposing credentials
    Email.send({
...
function submitRecipe(event) {
  ...
  if (recipe.value !== "") {
    Email.send({
      ...
      Swal.fire({
        title: "Error!",
        text: "Failed to send recipe. Please try again.",
        icon: "error"
      });
